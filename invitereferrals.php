<?php
/*
    Plugin Name: Customer Referral Program | Refer a Friend Software
        ...
        */
        public function invitereferrals_scripts()
        {
            if (!is_admin()) {
                ...
                $encryptedkey = trim($options['encryptedkey']);
                ...
                if ($options['enable_rewards']) {
                    $this->show_invitereferrals_reward_js($encryptedkey,$brandID,$secretkey);
                }
            }
        }
        
        public function show_invitereferrals_reward_js($encryptedkey="",$brandID="",$secretkey="")
        {        	
			...
			$md5SecretKey = strtoupper(md5($secretkey.'|'.$bid.'|'.$t.'|'.$setUserEmail));
